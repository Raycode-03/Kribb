...
const PRICE_PRESETS = [
  { label: "Under #50", min: null, max: 5000000 },
...
const chipText = (active: boolean) =>
  `text-sm font-semibold ${active ? "text-whtie" : "text-gray-600"}`;
...
export default function FilterModal({
  ...
  return (
    ...
    >
      <View
        className="flex-1 items-center justify-between px-5 pt-6  
          ...
          <View className="flex-row flex-wrap gap-2 mb-6">
            {BEDS.map((item) => (
              <TouchableOpacity
                ...
                className={`flex-1 items-center py-3 rounded-2xl border ${chip(type === item.value)}`}
                style={shadow}
              >
                <Text
                  className={`text-sm font-bold $chipText(type===item.value)`}
                >
                  {item.label}
                </Text>
              </TouchableOpacity>
            ...
            {PRICE_PRESETS.map((p) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  className={`px-3 py-1.5 rounded-full border items-center
                  ${
                    active
